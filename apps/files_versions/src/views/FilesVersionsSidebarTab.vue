				...
				<ul :aria-label="t('files_versions', 'File versions')" data-files-versions-versions-list>
					<template v-if="visibleSections.length === 1">
						<VersionEntry
							...
							:node="node"
							:is-current="row.items[0].version.mtime === node.mtime?.getTime()"
...
watchEffect(async () => {
	...
	try {
		loading.value = true
		versions.value = await fetchVersions(node)
	} finally {
		loading.value = false
	}
})
...
function compareVersion({ version }: { version: Version }) {
	...
	window.OCA.Viewer.compare({ path: props.node!.path }, _versions.find((v) => v.source === version.source))
}
</script>
